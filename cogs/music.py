...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if processed_info is None:
            raise YTDLError('Не мог принести `{}`'.format(webpage_url))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Сейчас играет',
                 ...
                 .add_field(name='Запрошен от', value=self.requester.mention)
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Эта команда не может быть использована в DM каналах.')
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Вы не подключены к голосовому каналу или не указали канал для подключения.')
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Сейчас я не играю никакой музыки...')
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Пропуск песни в голосование добавлен, в настоящее время **{}/3**'.format(total_votes))
...
def setup(bot):
    ...
    print("[SplashBot] Ког: Музыка. Загружен")